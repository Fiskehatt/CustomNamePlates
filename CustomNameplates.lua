...
local helthBarHeight = 7;
local cnp_UpdateInterval = 0.1; -- How often the OnUpdate code will run (in seconds), affects performance
local cnp_TimeSinceLastUpdate = 0;
...
local function cnpUpdate(self, elapsed)
	local time = GetTime()
  if (cnp_TimeSinceLastUpdate < time) then
		CustomNameplates_OnUpdate()
		--DEFAULT_CHAT_FRAME:AddMessage("update")
		cnp_TimeSinceLastUpdate = time + cnp_UpdateInterval
  end
end

local cnpTimer = CreateFrame("frame")
cnpTimer:SetScript("OnUpdate", cnpUpdate)
